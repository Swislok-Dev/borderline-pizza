        ...
        if (val === 'standard') {
          ...
          result.push(
            <div key={val} className="">
              <p className="text-gray-100 font-semibold text-left">
  ...
  const showOptionFields = (currentValue) => {
    return (
      <div key={currentValue} className="flex justify-between">
        <h4 className="italic text-left">{currentValue[0]}</h4>
  ...
  return (
    <Link href={`/product/${slug}`}>
      ...
      >
        <div
          key={slug}
          className="rounded-md p-2 m-auto my-4 max-w-screen-lg bg-blue-300 hover:bg-blue-600 hover:text-gray-200 active:text-red-300"
        >
          {showFields(prices)}
          {showOptions(options)}
        </div>
